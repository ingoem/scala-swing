...
class ListView[A] extends Component with CellView[A] with RenderableCells[A] {
  ...
  object selection extends CellSelection {
    ...
    peer.getSelectionModel.addListSelectionListener(new ListSelectionListener {
      def valueChanged(e: javax.swing.event.ListSelectionEvent) {
        // TODO Reinstate for the real ListView
  ...
  peer.getModel.addListDataListener(new ListDataListener {
    def contentsChanged(e: ListDataEvent) {} // { publish(ListChanged(ListView.this)) } // TODO Reinstate for the real ListView
    def intervalRemoved(e: ListDataEvent) {} //{ publish(ListElementsRemoved(ListView.this, e.getIndex0 to e.getIndex1)) } // TODO Reinstate for the real ListView
    def intervalAdded(e: ListDataEvent) {} //{ publish(ListElementsAdded(ListView.this, e.getIndex0 to e.getIndex1)) } // TODO Reinstate for the real ListView
  })
}
